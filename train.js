...
class CityStat {
  constructor(csvStr) {
    this.csvStr = csvStr
    this.array = this.convertCsvToArray(csvStr)
  }
  convertCsvToArray(csv) {
    return csv.replace(/\s+/g, '').replace(/^,+|,+$/g, '').split(/,/)
  }
  countWordLength() {
    let dict = {}
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if (Object.prototype.hasOwnProperty.call(dict, len)) {
        dict[len].num += 1
        dict[len].items.push(array[i])
      } else {
        dict[len] = {
          num: 1,
          items: [array[i],]
  ...
  countCharFrequency() {
    let str = this.csvStr.replace(/[\s,]/g, '')
    let dict = {}
  ...
  getCityArrayIncludingChar(char, onlyMatchFirst = true) {
    let array = []
    for (let i = 0; i < this.array.length; i++) {
      let city = this.array[i]
      if (onlyMatchFirst) {
        if (city.indexOf(char) === 0) {
          array.push(city)
        }
      } else {
        if (city.includes(char)) {
          array.push(city)
        }
      }
    }
  ...
  makeWord(word, onlyMatchFirst = true) {
    if (word.length < 2) {
      log('太短了')
      return
    }
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      wordDict[char] = {
        char,
        cityArr: this.getCityArrayIncludingChar(char, onlyMatchFirst)
      }
    }
    log(wordDict)
    return wordDict
...
let c = new CityStat(str)
// c.countWordLength()
// c.countCharFrequency()
c.makeWord('朝辞白帝彩云间', false)
// c.makeWord('桑益民')
// c.makeWord('马辉', false)
